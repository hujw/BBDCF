...
export function replaceNullToEmptyField(name, value, isInteger) {
  return isInteger ? `${name}: ${value},` : ![null, undefined].includes(value) ? `${name}: "${value}",` : '';
}
...
export function base64ToFile(b64, name, type) {
  return new File([base64ToBlob(b64)], name, { type });
}
...
export function convertMiliseconds(miliseconds, format) {
  ...
  switch (format) {
    ...
    default: {
      let str = '';
      if (days !== 0) {
        str += `${days < 10 ? '0' + days : days} D `;
      }
      str += `${hours < 10 ? '0' + hours : hours} H ${minutes < 10 ? '0' + minutes : minutes} M `;
      if (seconds !== 0 && days === 0) {
